...
def createPage():
   ...
   # st.title('대여소 간 거리 계산이 필요하신가요?')
    ...
    def filter_data():
        ...
        st.session_state.filtered_df['LAT'] = st.session_state.filtered_df['LAT'].astype(float)
        st.session_state.filtered_df['LOT'] = st.session_state.filtered_df['LOT'].astype(float)

        st.session_state.filtered_df2['LAT'] = st.session_state.filtered_df2['LAT'].astype(float)
        st.session_state.filtered_df2['LOT'] = st.session_state.filtered_df2['LOT'].astype(float)

    if st.button('조회'):
        filter_data()

    if not st.session_state.filtered_df.empty and not st.session_state.filtered_df2.empty:
        ...
        start_station = st.selectbox('출발 대여소를 선택하세요', st.session_state.filtered_df['ADDR1'])
        end_station = st.selectbox('도착 대여소를 선택하세요', st.session_state.filtered_df2['ADDR1'])

        start_lat = st.session_state.filtered_df[st.session_state.filtered_df['ADDR1'] == start_station].iloc[0]['LAT']
        start_lon = st.session_state.filtered_df[st.session_state.filtered_df['ADDR1'] == start_station].iloc[0]['LOT']

        end_lat = st.session_state.filtered_df2[st.session_state.filtered_df2['ADDR1'] == end_station].iloc[0]['LAT']
        end_lon = st.session_state.filtered_df2[st.session_state.filtered_df2['ADDR1'] == end_station].iloc[0]['LOT']
        ...
        if st.button('거리 계산'):
            ...
            pdk.Layer(
                'ScatterplotLayer',
                data=data,
                get_position='[lon, lat]',  # Correct order: longitude first, then latitude
